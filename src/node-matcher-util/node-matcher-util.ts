...
export class NodeMatcherUtil implements INodeMatcherUtil {
	...
	private matchNodeWithNode<T extends Node> (node: T, matchNode: Node): boolean {
		...
		else if (isConditionalTypeNode(matchNode)) {
			return this.matchNodeWithConditionalTypeNode(node, matchNode);
		}
	...
	/**
	 * Matches the provided node with the provided ConditionalTypeNode
	 * @param {Node} node
	 * @param {ConditionalTypeNode} matchNode
	 * @returns {boolean}
	 */
	private matchNodeWithConditionalTypeNode (node: Node, matchNode: ConditionalTypeNode): boolean {
		// If the node is not a ConditionalTypeNode, return false
		if (!(isConditionalTypeNode(node))) return false;

		return this.allIsMatched(
			this.matchTypeNode(node, matchNode),
			this.matchNodeWithTypeNode(node.checkType, matchNode.checkType),
			this.matchNodeWithTypeNode(node.extendsType, matchNode.extendsType),
			this.matchNodeWithTypeNode(node.trueType, matchNode.trueType),
			this.matchNodeWithTypeNode(node.falseType, matchNode.falseType)
		);
	}
